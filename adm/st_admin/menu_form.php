...
function add_menu_list(name, link, code)
{
	...
	if( code.length == 4 )
		sub_menu_class = " class=\"td_category sub_menu_class third\"";	
    <?php } ?>

    var list = "<tr class=\"menu_list menu_group_<?php echo substr($code, 0, 2); ?> sub_menu_group_<?php echo substr($code, 0, 4); ?>\">";
    list += "<td"+sub_menu_class+">";
    list += "<label for=\"me_name_"+ms+"\"  class=\"sound_only\">메뉴<strong class=\"sound_only\"> 필수</strong></label>";
    list += "<input type=\"hidden\" name=\"code[]\" value=\"<?php echo $new == 'new'? $code: $code.'00'?>\">";
    ...
    list += "<option value=\"2\">관리자만</option>";
    ...
    list += "</td>";
    list += "<td class=\"td_mngsmall\">";
    list += "<input type=\"hidden\" name=\"me_meta_title[]\" value=\"\">";
    list += "<input type=\"hidden\" name=\"me_meta_description[]\" value=\"\">";
    list += "<input type=\"hidden\" name=\"me_meta_keywords[]\" value=\"\">";
    list += "<input type=\"hidden\" name=\"me_meta_robots[]\" value=\"\">";
    list += "<input type=\"hidden\" name=\"me_meta_image[]\" value=\"\">";
    list += "<input type=\"hidden\" name=\"me_addinfo[]\" value=\"\">";	
    <?php if( $new == 'new' ) { ?>
    ...
    if(code) {
        ...
        if( code.length == 4 )
			$menu_last = $menulist.find("tr.sub_menu_group_"+code+":last");		
	}
    else
